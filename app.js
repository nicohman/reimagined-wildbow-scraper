...
var scrapeChap = function(url, name){
  https.get({hostname:url[0], path:url[1], port:443, agent:undefined}, function(res){
    var agg = '';
...
if(program.resume){
  fs.readFile('./'+program.resume, function(err, data){
    ...
    https.get({hostname:data.last[0], path:data.last[1], port:443, agent:undefined}, function(res){
      var agg = '';
      res.on("data", function(bit){
        agg +=bit;
      });
